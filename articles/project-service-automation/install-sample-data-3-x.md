...
4. Jika anda tidak menggunakan Spencer Low dengan nama pengguna **spencerl**, anda perlu mengemas kini satu fail tambahan. Buka fail **DemoDataPreImportConfig.xml**, dan kemudian cari tag **userstocreateandconfigure**. Kemas kini tag **\<log masuk\>** dengan nama pengguna bagi pengguna Spencer Low. Untuk maklumat tambahan, lihat [Nota teknikal](#technical-notes).
...
4. Jika pengguna nama penuh "Spencer Low" anda mempunyai ID pengguna yang berbeza daripada **"spencerl"**, maka anda perlu mengemas kini satu fail tambahan. Buka **DemoDataPreImportConfig.xml** dan kemudian cari tag **userstocreateandconfigure**. Kemas tag **\<log masuk\>** dengan loginId (sensitif huruf). 
...
- **\<RequiredSolutions\>** menentukan pemasangan penyelesaian pra-syarat dan nombor versinya.
...
- **\<EntitiesToEnableScheduling\>** menentukan entiti yang sepatutnya didayakan untuk Tempahan dalam Penjadualan Microsoft Dynamics (juga dikenali sebagai Universal Resource Scheduling).
...
- **\<PluginsToDisable\>** menentukan pasang masuk item baris yang sangat berhati-hati yang mesti dinyahdayakan semasa import data sampel dan kemudian mendayakan semula selepas itu.
